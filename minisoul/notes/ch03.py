...
def _(mo):
    ...
    ![3-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-1.svg)

    ## 3.1 编写LLM架构

    ![3-2](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-2.svg)
...
def _(mo):
    ...
    + `context_length`: 表示模型的最大输入标记数，由第 1 章中介绍的位置嵌入启用
    + `emb_dim`: 标记输入的嵌入大小，将每个输入标记转换为 768 维向量
    ...
    ![3-3](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-3.svg)
    """)
    return
...
def _(mo):
    mo.md(r"""
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-4.svg)
    """)
    return
...
def _(mo):
    ...
    [📝arXiv](https://arxiv.org/pdf/1607.06450)

    > 层归一化，简称 **LayerNorm**。是一种对神经网络层内的特征进行归一化的技术，用于加速训练、稳定梯度并提高模型的泛化能力。
    与 **BatchNorm** 不同，LayerNorm 是对**每个样本的特征维度**进行归一化，而不是对一个 batch 的样本进行统计。
    ...
    + 其中 $d$ 表示该层的特征维度（如 Transformer 中的 hidden size）。
    ...
    | :---: | :---: | :---: |
    | $x_i$               | 输入特征的第 $i$ 个分量            | |
    | $\mu$               | 当前样本在特征维度上的均值             | $$\mu = \frac{1}{d} \sum_{i=1}^{d} x_i$$ |
    | $\sigma^2$          | 当前样本在特征维度上的方差             | $$\sigma^2 = \frac{1}{d} \sum_{i=1}^{d} (x_i - \mu)^2$$ |
    | $\epsilon$          | 为防止除零引入的微小常数（如 $10^{-5}$） | |
    | $\hat{x}_i$         | 归一化后的结果（均值为 0，方差为 1）      | $$\hat{x}_i = \frac{x_i - \mu}{\sqrt{\sigma^2 + \epsilon}}$$ |
    | $\gamma_i, \beta_i$ | 可学习的缩放与平移参数（同维度 $d$）      | |
    | $y_i$               | LayerNorm 输出结果            | $$y_i = \gamma_i \hat{x}_i + \beta_i$$ |


    + LayerNorm 的核心思想是： “对单个样本内部的特征做归一化，让每个样本的特征分布更稳定。”

    > + 如果某一层输出 $x = [10, 12, 14]$，均值为 12，方差为 4。那么归一化后：
    > + $\hat{x} = \frac{x - 12}{\sqrt{4}} = [-1, 0, 1]$
    > + 再通过 $\gamma, \beta$ 进行缩放与平移得到最终输出。
    ...
    | ------------- | ------------- | --------------------- |
    | 归一化维度         | batch 方向（跨样本） | 特征方向（单样本）             |
    | 是否依赖 batch 大小 | 是             | 否                     |
    | 应用场景          | CNN 等结构       | RNN、Transformer 等序列模型 |
    | 计算均值方差        | 对同一特征在多个样本间求  | 对同一样本的所有特征求           |
    ...
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-05.svg)
...
def _(mo):
    ...
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-6.svg)
...
def _(mo):
    mo.md(r"""
    :hammer: 上面，我们对每个输入的特征进行了归一化。现在，使用相同的思路，我们可以实现一个 LayerNorm 类：
    """)
    return
...
@app.cell(hide_code=True)
def _(LayerNorm, batch_example, mo):
...
def _(mo):
    mo.md(r"""
    ![3-7](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-7.svg)

    ## 3.3 GELU激活实现FNN

    > [:link: Gaussian Error Linear Units (GELUs)](https://arxiv.org/pdf/1606.08415) 本节内容来介绍一下GELU激活函数。
...
def _(mo):
    ...
    > 前馈网络（Feedforward Neural Network, FNN）中的 Feedforward 模块的标准结构如下：

    $$
    FFN(X) = W_2 \cdot \sigma(W_1 \cdot X + b_1) + b_2
    $$

    其实可以简单理解为先经过一个线性层$L_1$，在经过一个激活函数$\sigma$，最后在经过一个线性层$L_2$:
    """)
    return
...
def _(mo):
    mo.mermaid("""
    graph LR;
    A(X) --> L1("Linear Layer 1") --> GELU("GELU") --> L2("Linear Layer 2") --> B(Y)
    """)
    return
...
def _(mo):
    mo.md(r"""
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-8.svg)
    """)
    return
...
def _(mo):
    mo.md(r"""
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-9.svg)

    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-10.svg)
    """)
    return
...
def _(mo):
    ...
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-011.svg)
...
def _(mo):
    mo.md(r"""
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-12.svg)
...
def _(mo):
    mo.md(r"""
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-13.svg)
    ...
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-14.svg)
...
def _(mo):
    mo.md(r"""
    :rocket: 使用 124M 参数模型的配置，我们现在可以使用随机初始权重实例化此 GPT 模型，如下所示：
    """)
    return
...
def _(mo):
    mo.md(r"""
    :hammer: 我们将在下一章训练这个模型。不过，需要简单说明一下它的大小：我们之前称它是一个 124M 参数的模型；我们可以用以下方法再次确认这个数字：
    """)
    return
...
def _(mo):
    ...
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-15.svg)
    ...
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-16.svg)
    """)
    return
...
def _(mo):
    ...
    ![1-1](https://codingsoul-images.tos-cn-beijing.volces.com/LLM/3-17.svg)
...
def _(mo):
    mo.md(r"""
    :fire: 请注意，该模型尚未训练；因此上面的输出文本是随机的。 我们将在下一章训练该模型。
    """)
    return
